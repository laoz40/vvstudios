...
const SESSIONS_DASHBOARD_STORAGE_KEYS = {
...
function getSessionsDashboardStorage() {
	return typeof window === "undefined" ? null : window.localStorage;
}

function readStoredBoolean(key: string, fallback: boolean) {
	const value = getSessionsDashboardStorage()?.getItem(key);
...
export function readStoredSessionsDashboardSorting() {
	const value = getSessionsDashboardStorage()?.getItem(SESSIONS_DASHBOARD_STORAGE_KEYS.sorting);
...
export function readStoredShowUpcomingOnly() {
	return readStoredBoolean(
		SESSIONS_DASHBOARD_STORAGE_KEYS.showUpcomingOnly,
...
export function readStoredShowStaleBookings() {
	return readStoredBoolean(
		SESSIONS_DASHBOARD_STORAGE_KEYS.showStaleBookings,
		DEFAULT_SHOW_STALE_BOOKINGS
	);
}

export function storeSessionsDashboardSorting(sorting: SortingState) {
	getSessionsDashboardStorage()?.setItem(
		SESSIONS_DASHBOARD_STORAGE_KEYS.sorting,
